...
{
    public partial class TD_SerialDetail : Form
    {
        public TD_SerialDetail()
        {
            InitializeComponent();
        }
        ...
        private void TD_SerialDetail_Load(object sender, EventArgs e)
        {
            ...
            windowText = (sender as TD_SerialDetail).Text;
        }
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            if (resultNum1 > 0)
            {
                ...
                sumValue(this.dataGridViewX1, 4, 5);
                dateClear();
            }
            else
            {
        ...
        private void buttonX3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                sql1 = string.Format("update Pub_SerialDetail set " +
                    ...
                    "SerialNum = '{5}', GetAccount = '{6}', OperationPerson = '{7}',EnterType = '{8}',Remark = '{9}' where id = {0}",
                    this.dataGridViewX1.SelectedRows[0].Cells[0].Value.ToString(),
                    this.dateTimeInput1.Value.ToShortDateString(),
        ...
        private void buttonX4_Click(object sender, EventArgs e)
        {
            string sql1 = string.Format("select * from Pub_SerialDetail where CompanyNames = '{0}' and TableType = '{1}' " +
                ...
                " and SerialNum like '%{2}%' and OperationPerson like '%{5}%' and MatterRemark like '%{6}%' and Remark like '%{7}%'"+ 
                "",
        ...
        private void sumValue(DataGridViewX dgvx,int typeIndex,int index)
        {
            double getVal = 0;
            double payVal = 0;
            double sumVal = 0;
            try
            {
                for (int i = 0; i < dgvx.Rows.Count; i++)
                {
                    try
                    {
                        if (dgvx.Rows[i].Cells[typeIndex].Value.ToString() == "收入")
                        {
                            getVal += double.Parse(dgvx.Rows[i].Cells[index].Value.ToString());
                        }
                        else
                        {
                            payVal += double.Parse(dgvx.Rows[i].Cells[index].Value.ToString());
                        }
                    }
                    catch { sumVal += 0; }
                }
                this.labelX12.Text = "收入 ： " + tools.doubleToInt(getVal.ToString()).ToString() + " 元";
                this.labelX13.Text = "支出 ： " + tools.doubleToInt(payVal.ToString()).ToString() + " 元";
                this.labelX20.Text = "合计金额 ： " + tools.doubleToInt((getVal - payVal).ToString()).ToString() + " 元";
            }
            catch(Exception ex)
            {
